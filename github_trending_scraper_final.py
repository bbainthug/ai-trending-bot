...
def exclude_specific_repos(repos, exclude_list=None):
    """
    排除特定的仓库
    
    Args:
        repos: 仓库列表
        exclude_list: 要排除的仓库名称列表
        
    Returns:
        list: 排除特定仓库后的列表
    """
    if exclude_list is None:
        exclude_list = ["openclaw/openclaw"]  # 默认排除openclaw
    ...
    for repo in repos:
        ...
        for exclude_repo in exclude_list:
            if exclude_repo.lower() in repo["name"].lower():
                should_exclude = True
                break
...
def main():
    ...
    # 排除特定仓库（如openclaw）
    ai_repos = exclude_specific_repos(ai_repos, ["openclaw/openclaw"])
    print(f"🔍 排除特定仓库后剩余 {len(ai_repos)} 个仓库")
    
    # 创建Markdown表格
    markdown_content = create_markdown_table(ai_repos)
    
    # 保存到文件
    if save_to_file(markdown_content, "github_trending_ai.md"):
        # 显示所有仓库的简要信息
        print("\n📋 AI相关仓库列表:")
        for i, repo in enumerate(ai_repos, 1):
            print(f"{i:2d}. {repo['name']:30} ★{repo['stars']:>6} - {repo['description'][:50]}...")
    else:
        print("❌ 保存失败")
